 ...
 *    advertencia que indica la ruta y el límite (Requirement 6.1).
 ...
 *    comparación exacta de nombre (Requirement 6.2).
 ...
 *    descartan cuando apuntan fuera (Requirement 6.3).
 *  - El contenido se lee solo aquí, durante la resolución de una ejecución: el
 *    descubrimiento del catálogo no llama a este módulo (Requirement 6.4).
...
/** Destino de las fuentes inline en el entorno remoto (Requirement 6.1). */
...
/** Único motivo por el que la carga de fuentes inline falla (Requirement 6.7). */
 ...
 * archivos de ejecución (Requirement 6.4).
 *
 * Lanza `AgentInlineSourcesError` si el conjunto queda vacío tras aplicar las
 * exclusiones y los límites (Requirement 6.7).
 */
export function loadAgentInlineSources(
  ...
  /**
   * Requirement 6.2: se excluyen `manifest.json` y los nombres declarados en
  ...
  const walk = (dir: string, relativeDir: string, depth: number): void => {
    ...
    for (const name of names) {
      ...
      // Requirement 6.3: la ruta resuelta debe quedar dentro de la carpeta del
      ...
      if (stat.isDirectory()) {
        // Requirement 6.1: profundidad máxima de 5 niveles de subcarpetas.
      ...
      // Requirement 6.2: metadata del servidor, nunca se sube.
      if (excludedFileNames.has(name)) continue;

      // Requirement 6.1: máximo de 200 archivos por agente.
      ...
      // Requirement 6.6: 1 MB por archivo.
      ...
      // Requirement 6.5: un archivo ilegible no invalida el resto.
      let content: string;
      try {
      ...
      sources.push({
        ...
        content,
        // Requirement 6.1: destino `/.agents` preservando la ruta relativa.
  ...
  // Requirement 6.7: sin fuentes inline no hay ejecución posible.
